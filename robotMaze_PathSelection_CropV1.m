...
platCentre = nan(3,2);
for p = 1:3
    platCentre(p,:) = platCoor(robot(p).pos).Centre;
end
platCentre = mean(platCentre, 1);

cropWidth = 500;
cropHeight = 400;

cropNums = [platCentre(1); platCentre(2); cropWidth; cropHeight];
...
while ~atGoal
    ...
    platCentre = nan(6,2);
    allPos = [vertcat(robot(:).pos); nextPlats'];

    for p = 1:6
        platCentre(p,:) = platCoor(allPos(p)).Centre;
    end
