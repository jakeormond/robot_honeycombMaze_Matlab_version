% robotMaze_PathSelection_Tracking_TESTING
...
while ~atGoal    
    ...
    for e = 1:nEpochs
        ...
        [robot, turnOnlyFlag] = sendSSHcommands_simple_TESTING(robot, [robotInput{1}(e), robotInput{2}(e)], ...
        ...
        if turnOnlyFlag && (checkAdjacent(robot(statRobot).pos, robot(movingRobots(1)).pos, platformMap) ...
            ...
            minPlatTime = 1;
            platform = getDecision_WaitBar(prevStartPlat, prevChoicePlats, platCoor, cropNums, minPlatTime); % currentPlat 
            if ~isempty(platform)               
