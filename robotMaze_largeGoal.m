...
[goal_rings, ~] = getRings(goalPlatform, platformMap);
all_goal_plats = goal_rings.inner;
%% set data storage
...
while ~atGoal    
    ...
    if ~isempty(futurePlats) % i.e. rat has changed his mind twice, so use previous set of nextPlats
        ...
        if atGoal % if animal has reached the goal, just move choice robots away
            ...
            nextPlats = pickNextPlatforms_largeGoal(prevChoices, robot, platformMap, ... % pick the new ones
