...
% robotCh{3}  =  sshfrommatlab('root', '192.168.0.104','');
...
robot(3).pos = 119;
robot(3).dir = 60;
robot(3).ip = [192, 168 ,0, 104];

statRobot = 3; % stationary robot, other robots are to move to adjacent positions.
...
choiceCounter = 0; % NEED TO POPULATE THIS STILL!!!!!!!!!!!!!!!
...
while ~atGoal    
    ...
    for p = 1:2
        ...
        end % outer target is where robot MUST reach before moving to final position
        [oTargetRings, oTargetVertices] = ...
            getRings(outerTargets, platformMap);
        ...
        for r = 1:2
            ...
            if middleRingFlag
                ...
                % NEED TO CHECK: 1. THIS IS WRONG: Is the robot on a middle vertex of the target,
...
%                             nextPos; nextPlats(p)];
                    ...
                    if checkAdjacent(initialPos{r}(end), outerTargets, platformMap)
                        pathFromInit{r, p, d} = [initialPos{r}; ...
                            outerTargets; nextPlats(p)];
                    else
                        ...
                        while true
                            ...
                            if strcmp(ring, 'outer')
                                ...
                                % determine if path intesects outer target
%                                 [~, targetVertInd] = intersect(pathTemp{counter}, targetVertices.middle);
% 
%                                 if ~isempty(targetVertInd)
%                                     targetVertInd = targetVertInd(1);
                                oTargetInd = find(pathTemp{counter} == outerTargets);    
                                if ~isempty(oTargetInd)
                                    pathTemp{counter} = pathTemp{counter}(1:oTargetInd);
                                     
                                    pathFromInit{r, p, d} = ...
                                        [vertcat(pathTemp{:}); nextPlats(p)];
                                    break
                                end
                                 
                                adj2outer = false(length(pathTemp{counter}), 1);
                                for i = 1:length(pathTemp{counter})
                                    adj2outer(i) = checkAdjacent(pathTemp{counter}(i), ...
                                        outerTargets, platformMap);
                                end
                                if ~isempty(find(adj2outer, 1))
                                    adjInd = find(adj2outer, 1);
                                    pathTemp{counter} = pathTemp{counter}(1:adjInd);
                                    pathFromInit{r, p, d} = ...
                                        [vertcat(pathTemp{:}); outerTargets; nextPlats(p)];
                                    break                                    
...
%                                     break
    ...
    for p1 = 1:2 % robot1's platform, which determines robot2's platform
        ...
        for d1 = 1:2 % robot1's direction
            ...
            if length(initialPos{1}) == 1
                startInd1 = find(pathsTemp{1} == initialPos{1});
            else
                startInd1 = find(pathsTemp{1} == initialPos{1}(2));
            end
            pathsTemp{1} = pathsTemp{1}(startInd1:end-1);
            
            for d2 = 1:2 % robot2's direction                
                pathsTemp{2} = pathFromInit{2, p2, d2};
                
                if length(initialPos{2}) == 1
                    startInd2 = find(pathsTemp{2} == initialPos{2});
                else
                    startInd2 = find(pathsTemp{2} == initialPos{2}(2));
                end
                pathsTemp{2} = pathsTemp{2}(startInd2:end-1);
        ...
        for d1 = 1:2 % robot1's direction
            ...
            for d2 = 1:2 % robot2's direction                
                ...
                if ~compMatrix(p1, d1, d2)                    
                    ...
                    if staggerMatrix(p1, d1, d2) == 0 || isnan(staggerMatrix(p1, d1, d2))
                        maxLengths(p1, d1, d2) = max(length(pathsTemp{1}), ...
                            length(pathsTemp{2}));
                    else
                        maxLengths(p1, d1, d2) = max(length(pathsTemp{movFirst}), ...
                            length(pathsTemp{movSecond}) + staggerMatrix(p1, d1, d2));
    ...
    staggers = NaN(length(p1), 1);
    for i = 1:length(p1)
        staggers(i) = staggerMatrix(p1(i), d1(i), d2(i));
    end
    [~, minInd] = min(staggers);  
    
    pathsFinal{1} = pathFromInit{1,p1(minInd),d1(minInd)};
    p2 = find(~ismember([1 2], p1(minInd)));
    pathsFinal{2} = pathFromInit{2,p2,d2(minInd)};
    ...
    % position away as soon as the first robot has moved some distance UNLESS
    % it is not adjacent to the stationary platform.
%     if movFirst ~= 0
%         movSecond = find(~ismember([1,2], movFirst));
%     else
%         movSecond = 0;
%     end
    ...
    for r = 1:2
        ...
        % if directions(1) ~= currDir  % THIS IS NOT RIGHT IF THE ROBOT CAN TURN AROUND IN PLACE
        if abs(currDir - directions(1)) == 180 
            turnAround(r) = 1;
        end
        ...
        if ~ismember(abs(currDir-directions(1)), [0 180]) 
            turns = [(directions(1) - currDir); turns];            
        end        
        
        turns(turns < 0) = turns(turns < 0) + 360;
        turnsLines{r} = turns/60;
        
        if r == movSecond && (abs(currDir - directions(1)) == 180 || ...
                currDir - directions(1) == 0)
            % there will be 3 segments: 1) the first half-move,
            ...
            if abs(currDir - directions(1)) == 180 || ...
                ...
                for s = 1:nSegments
                    if s == 1
                        robotInput{r}{1}(2) = 0;    
                        robotInput{r}{1}(3) = turnsLines{r}(1);
                        robotInput{r}{1}(4) = segmentLengths{r}(1);  
                    else
                        robotInput{r}{1}(s*2 + 1) = turnsLines{r}(s);
                        if s < nSegments
                            robotInput{r}{1}(s*2 + 2) = segmentLengths{r}(s);
                        end
                    end                  
                end
                if segmentLengths{r}(end) ~= 1
                    robotInput{r}{1}(nSegments*2 + 2) = segmentLengths{r}(end)-1;
                end
            end
    ...
    if movFirst > 0
        ...
        if stagger > 1
            ...
            if turnAround(movFirst) == 1
                timing{movSecond}(2) = timing{movSecond}(2) + 4;
            end
        ...
        elseif robotInput{movSecond}{1}(2) == 97
            ...
            robotInput{movSecond}{1} = [robotInput{movSecond}{1}(1), ... % THIS IS ALSO WRONG AS IT 
                1, robotInput{movSecond}{2}(3:end)];                     % DOESN'T APPLY TO NON-ADJACENT
            robotInput{movSecond}(2) = [];       
            % PLATFORMS
            
        elseif robotInput{movSecond}{1}(2) == 0 % this is when one platform is turning in place
            % THESE LINES ARE IDENTICAL TO ABOVE, SO COMBINE ONCE
            % WORKING!!!!
            turnsFirst = turnsLines{1};
            turnsFirst(turnsFirst > 3) = 6-turnsFirst(turnsFirst > 3);
            totalTimeFirst = sum(segmentLengths{1})*timePerGap + sum(turnsFirst)*timePerLine;
