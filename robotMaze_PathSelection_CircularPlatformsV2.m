...
robot(2).pos = 148; % robots 2 and 3 should be placed 1 spot away from this position
...
% close to walls
excludedPlatsCell{1} = platformMap(1:3, :); restrictedMap(1:3, :) = [];
...
while ~atGoal
    %% first, move the "moving" robots into position.
%     tic
%     for r = 1:2
%         ipAddr = robot(nonStatRobots(r)).ip;
%         robotCom = './honeycomb_fast 0 1';
...
%         end
    ...
    for r = 1:2
        ...
        if isempty(plats2turns)
            segmentLengths{r} = length(pathsFinal{r}) - 1;
        ...
        if isempty(plats2turns)           
            if segmentLengths{r} == 1
                robotInput{r}{1} = [robotInput{r}{1}, 1];
            else
                robotInput{r}{1} = [robotInput{r}{1}, segmentLengths{r} -1];
                robotInput{r}{2} = [0 1];
            end
    ...
    for e = 1:nEpochs
        ...
        if e == 1 && nEpochs == 3 % if we are on a turn-only step
            decChange = input('did the animal change it''s decision? (''y'', hit enter for no])', 's');
