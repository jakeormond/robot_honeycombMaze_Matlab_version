% robotMaze_PathSelection_Tracking
...
while ~atGoal    
    ...
    for e = 1:nEpochs
        ...
        [robot, turnOnlyFlag] = sendSSHcommands_simple(robot, [robotInput{1}(e), robotInput{2}(e)], ...
            pathsFinal, [duration{1}(e), duration{2}(e)], robotCh);

        % if that was a turn only step, we need to ask if the animal moved
        if turnOnlyFlag && (checkAdjacent(robot(statRobot).pos, robot(movingRobots(1)).pos, platformMap) ...
                || checkAdjacent(robot(statRobot).pos, robot(movingRobots(2)).pos, platformMap))
            ...
            minPlatTime = 1;
            [platform, exitKey] = getDecision_wExit(prevStartPlat, prevChoicePlats, platCoor, cropNums, minPlatTime); % currentPlat 
            if isempty(exitKey)               
            ...
            if e == nEpochs && ~atGoal % need to get animal's decision
                ...
                % OR
                minPlatTime = 4;
                [newStatPlat, exitKey] = getDecision_wExit(currPlat, nextPlats, platCoor, cropNums, minPlatTime); % currentPlat
                if ~isempty(exitKey)
                    newStatPlat = getManualDecision(robot, nextPlats, spinFlag);
                end
